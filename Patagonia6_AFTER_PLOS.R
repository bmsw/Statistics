...
# GVLMA
gvlma(lm(DATA3$FC_Shannon~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) #OK
gvlma(lm(DATA3$FC_Pielou~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) #OK
gvlma(lm(DATA3$FC_Richness~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) # NOT OK AT 0.05

# Log
gvlma(lm(log10(DATA3$FC_Richness)~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) # NOT OK

# ANOVA
aov.shannon=aov(lm(DATA3$FC_Shannon~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) # SIGNIFICANTE
aov.pielou=aov(lm(DATA3$FC_Pielou~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) # NOT SIGNIFICANTE

# Richness log for ANOVA
aov.rich=aov(lm(log10(DATA3$FC_Richness)~DATA3$`TROPHIC STATE`+env2$`Long. (°W)`+env2$`Lat. (°S)`)) # SIGNIFICANTE
summary(aov.shannon)
summary(aov.pielou)
summary(aov.rich)
# Tukey
...
# Adjusting labesl
label1=gsub("MESO_EUT", "MESOEUTROPHIC", DATA3$`TROPHIC STATE`)
...
mybox("FC_Pielou", "Pielou Index")
mybox("FC_Richness", "Richness")
mybox("Log_Richness", "Richness")
...
#       panel.grid.minor = element_blank(),
#       legend.title=element_blank(),
#       legend.direction = 'horizontal', legend.position = 'bottom', legend.text=element_text(size=12))+
...
#meta2=metaMDS(m1, autotransform = F, try = 200)
