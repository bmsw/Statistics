...
#' @importFrom estParamFlowVS transFlowVS flowVS
...
flowDiv<- function(myworkspaces, gate.name=NULL, ialpha="invsimpson", ibeta="bray", do.plot=FALSE, static=FALSE, dilutions=NULL,  pmax=0.05, env=NULL, use.beads=FALSE, beads=NULL, transform.hell=FALSE, dimension=20, psize=3, autotrans=FALSE){
  ...
  ops<-unique(unlist(lapply(unlist(nnn$nodesample), function(x)flowCore::colnames(x))))
  selection<-select.list(ops, multiple = T, title="Please select channels for use")
  ...
  binss=base::lapply(nnn$nodesample, function(x) base::lapply(x, function(y)base::lapply(selection, function(x)Freedman.Diaconis(flowCore::exprs(y)[,x]))))
  ...
  #MODIFICATION
  if (!is.null(dilutions))tables=mapply(function(x,y)x/(y/min(dilutions)), tables, dilutions, SIMPLIFY = F)
