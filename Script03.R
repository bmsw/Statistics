...
## Script 03/?? : Análise de Componentes Principais de Matrizes de Distâncias (PCNM)
####Pacotes utilizadoslibrary(ape)
library(spdep)
library(ade4)
library(adegraphics)
library(adespatial)
...
anova(capscale(mite.h~., data.frame(mite.xy), dist="euclidian"))	# Result: no significant trend for Hellinger
...
(mite.dbmem.rda <-capscale(mite.h~., mite.dbmem, dist="euclidian"))
