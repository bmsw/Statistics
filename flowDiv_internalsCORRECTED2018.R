# To do things:
## Standardize and then correct by divison
# Remove rare events - corrigir by dilution y depues sacar
# Correct dilutions by division: standardize by minor concentraitns
# Rodar flowDiv cambiando 1, 2 y 3 singlets: test!
# Use complete.cases with matrices
# Stres in NMDS
################################################
...
nn<- function(x, nod, nod2, use.beads)
{
  ...
  cols=unique(unlist(lapply(nodesample, function(x)flowWorkspace::colnames(x))))
 # print(cols)# It was inside else loop...Wrong
  if (use.beads){
  ...
  transdata=base::lapply(nodesample, function(k)base::mapply(function(x,y)flowCore::transform(x, y), k, meansCorrection))
  nodesample=transdata
  # Variance stabilization
  if (use.beads){
    samples.fS=flowSet(samples$nodesample[[1]])
    cofactors = estParamFlowVS(samples.fS,channels=selec)
    sample.VS = transFlowVS(samples.fS, channels=selec, cofactors)
    for(i in 1:length(samples.fS))samples$nodesample[[1]][[i]]<-samples.fS[[i]]
    rm(samples.fS)
  }
  ####
