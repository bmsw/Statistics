...
idds<-cem_lagos$ids[-94,1:3]

for (i in (1:ncol(m1))) {m1[,i]=ifelse(m1[,i]==0, NA,
                                               m1[,i])}
FDAll=Calculate.FD(landfile=t(m1),traitfile=idds)
QAll=Calculate.Q(landfile=data.frame(t(m1)), traitfile=idds)
FDAll
QAll
