...
# WD
setwd("/home/brunomsw/Documentos/DOUTORADO/DATA/Tara/Results")
# FASE 01: Importação e preparo dos datasets
#  Importar dataset OM_CompanionMODIFIED_ALL
OM_CompanionMODIFIED_ALL <- read.delim("/home/brunomsw/Documentos/DOUTORADO/DATA/Tara/TARA METADATA/OM.CompanionMODIFIED_ALL.csv", sep="\t", dec=",", strip.white = T)
...
######### flowDiv
tara.fd=flowDiv2("../Bruno_Tara_ALL_Bact10-4.wsp", "Bact", do.plot = F, use.beads = T,ialpha = "shannon", beads = "Beads", static = F, transform = F, autotrans=F, psize = 1)
...
g1=ggplot(aes(y=`FC - Richness`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+geom_point()
g1
g2=ggplot(aes(y=`FC - Pielou`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+geom_point()
g2
g3=ggplot(aes(y=`FC - Shannon`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+geom_point()
g3
g4=ggplot(aes(y=`FC - Simpson`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+geom_point()
g4
g5=ggplot(aes(y=`FC - invSimpson`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+geom_point()
g5
g6=ggplot(aes(y=`FC...bacteria..cells.mL.`, x=`sample.regions`), data=OM5)+geom_boxplot(aes(fill=`sample.regions`, alpha=.5))+ theme(legend.position="none")+ scale_x_discrete(name="")+ylab("FC Counts")
g6
...
ggplot(aes(MDS1, MDS2, shape=ocean.regions[-71], colour=sample.regions[-71]), data=data.frame(meta$points))+geom_point(size=3)+
  labs(shape="Ocean regions ", colour="Sample regions")
