...
## Sourcing data
source("/home/brunomsw/Documentos/DOUTORADO/DATA/Patagonia/Scripts&Data/DATA_IMPORT.R")
## Workings directly
setwd("/home/brunomsw/Documentos/DOUTORADO/DATA/Patagonia/flowDiv patagonia/Results")
...
# flowDiv
pata.fd=flowDiv2("../Romina_Bruno_1_10.wsp", "Bact tot", do.plot = F, static = F, dilutions=DATA$`x (flowDiv)`, transform = T, autotrans=T,psize = 1.08)
...
#     geom_text(aes(label=DATA3$Lago),hjust=-0.2, vjust=0, size=3)
#   return(g0)
# }
...
#M4=cbind(FC_Shannon=DATA3$FC_Shannon,FC_Pielou=DATA3$FC_Pielou, FC_Richness=DATA3$FC_Richness,FC_Counts=DATA3$FC_counts, env2[,c(2,4:9, 11, 14)])
#M4$`Chl a (μg l−1)`=log10(M4$`Chl a (μg l−1)`)
...
# wine.pca <- prcomp(M4[,c(1,3,10,11,13)], scale. = TRUE)
# wine.pca <- prcomp(M4[,-5], scale. = TRUE)
# wine.pca <- prcomp(M4[,c(1:4, 10:13)], scale. = TRUE)
wine.pca <- prcomp(M5[,c(1:4)], scale. = TRUE)

#levels(mylabels)<-c("MESOTROPHIC", "OLIGOTROPHIC", "EUTROPHIC")
ggbiplot(wine.pca, obs.scale = 1, var.scale = 1, varname.size=5, circle = F, groups = mylabels, ellipse = TRUE, ellipse.prob = 0.95) +
scale_color_discrete(name = '') +
  geom_point(aes(colour=mylabels),size = 3)+
theme_bw()+
theme(text = element_text(size=20),
      ...
      panel.grid.minor = element_blank(), 
      legend.title=element_blank(),
      legend.direction = 'horizontal', legend.position = 'bottom', legend.text=element_text(size=18))+
...
#nMDS - with an without aitotrans
...
# Linear variables
ordisurf(meta2~M5$pH) #
ordisurf(meta2~M5$`Chl a (μg L−1)`) #
ordisurf(meta2~M5$`DOC (ppm)`) #


# Non-linear variables (not used)
ordisurf(meta2~M5$`Lat. (°S)`)
ordisurf(meta2~M5$`Long. (°W)`)
ordisurf(meta2~ M5$`Altitude (m a.s.l.)`)
ordisurf(meta2~M5$`Area (km2)`)
ordisurf(meta2~ M5$`Temp.(°C)`)
ordisurf(meta2~M5$`Conduct. (μS cm−1)`)
ordisurf(meta2~ M5$`DO (mg L−1)`)
ordisurf(meta2~M5$`DIN (mg L−1)`)
ordisurf(meta2~M5$`Kd (m−1)`)

fit2=data.frame(fit$vectors$arrows)
rownames(fit2)<-c("Shannon","Pielou","Richness","Counts","Lat.","Lon.","Altitude","Area","Temp.","pH","Conduc.","DO","DIN","Kd","Chl a","Phos.","DOC")


ggplot(aes(NMDS1, NMDS2, colour=mylabels), data=data.frame(scores(meta2)))+
  geom_point(size=2)+
  geom_segment(data=fit2[c(10,14,17),], mapping=aes(x=0, y=0, xend=NMDS1, yend=NMDS2), arrow=arrow(length = unit(0.2, "cm")), size=.5, color="red4", linetype=1)+
  geom_text(data=fit2[c(10,14,17),]*1.1, aes(x=NMDS1, y=NMDS2, label=rownames(fit2[c(10,14,17),])), size=4, vjust=0,hjust=1, colour="red4")+
  theme_bw()+
